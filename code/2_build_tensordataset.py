...
def build_single_task_supervised_data(spark):
    ...
    for df, path in zip([trn, tst, hld], ['trn', 'tst', 'hld']):
        cvae.utils.mk_empty_directory(f'data/processed/single_task_tensors/{path}', overwrite=True)
        df.foreachPartition(lambda partition: create_tensors(partition, pathlib.Path(f'data/processed/single_task_tensors/{path}')))
