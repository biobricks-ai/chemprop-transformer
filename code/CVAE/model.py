...
class Encoder(nn.Module):
    def __init__(self, nSmilesChar, nLabels, lbldim, smidim, convdim):
        ...
        self.lbl_embed = nn.Embedding(nLabels, lbldim)
        self.smi_embed = nn.Embedding(nSmilesChar, smidim)
        
        self.relu = nn.ReLU()
        self.conv1 = nn.Conv1d(lbldim+smidim, convdim, kernel_size = self.kernelSize, stride = self.strideSize)
    ...
    def forward(self, smiEmbedding, labelsEmbedding):
        lblEmbedding = self.lbl_embed(labelsEmbedding)
        
        z = self.conv0(smiEmbedding)
        z = self.relu(z)
        z = self.conv1(z)
        z = self.relu(z)
        z = self.conv2(z)
        z = self.relu(z)
        z = z.view(-1)
        z = torch.cat((z, lblEmbedding), dim=0)
