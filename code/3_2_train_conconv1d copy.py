import pandas as pd, signal, numpy as np, tqdm, sys, os, pathlib
import torch, torch.nn.functional as F, torch.optim, torch.utils.data, torch.nn as nn
import faiss, numpy as np
import signal

sys.path.insert(0, os.getcwd())
import cvae.models.triplet, cvae.models.transformervae, cvae.utils, cvae.tokenizer.selfies_tokenizer
...
# load data ===========================================================================
keys = keys=['tselfies', 'tmorgan', 'tass', 'tval']
iselfies, imorgan, ilbl, ivalue = 0, 1, 2, 3
def load_ds(dir):
    ds = [torch.load(x) for x in dir.glob('*.pt')]
    return [ torch.cat([data[k] for data in ds]) for k in keys ]
    
tensor_dir = pathlib.Path("data/processed/activity_tensors")
tst, trn, hld = (load_ds(tensor_dir / x) for x in ['test.pt', 'train.pt', 'holdout.pt'])
...
# make model ===========================================================================
tvae = cvae.models.transformervae.TransformerVAE.load().to(device)
...
trndl = update_triplet(model, trn[iselfies], trn[ilbl], trn[ivalue]).mkloader()
tstdl = update_triplet(model, tst[iselfies], tst[ilbl], tst[ivalue]).mkloader()
...
trnemb = get_embeddings(model, trn[iselfies], trn[ilbl])
tstemb = get_embeddings(model, tst[iselfies], tst[ilbl])
hldemb = get_embeddings(model, hld[iselfies], hld[ilbl])
