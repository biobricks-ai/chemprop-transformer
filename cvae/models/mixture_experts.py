...
class MoE(nn.Module):
    ...
    def build_stratified_lossfn(self):
        ...
        ignore_index = -1
        # ce_lossfn = torch.nn.CrossEntropyLoss(reduction='none', ignore_index=ignore_index)
        ...
        # def lossfn(parameters, logits, output):
        #     batch_size, vocab_size, seq_len = logits.size()  # vocab_size in middle
        ...
        #     # Reshape logits so we can index correctly
        #     logits = logits.transpose(1, 2).contiguous()  # make logits shape [batch, seq_len, vocab] for masking
        #     logits_selected = logits[final_mask]  # (num_tokens, vocab)
        #     output_selected = output[final_mask]  # (num_tokens,)

        #     token_losses = ce_lossfn(logits_selected, output_selected)

        #     # value_positions = final_mask.nonzero(as_tuple=False)
        #     # batch_indices = value_positions[:, 0]
        #     # token_indices_v = value_positions[:, 1]

        #     # property_token_indices = token_indices_v - 1
        #     # property_token_ids = output[batch_indices, property_token_indices]

        #     # stratified_loss, unweighted_loss = self.property_loss_tracker.update(property_token_ids, token_losses, output.device)
        #     # total_loss = stratified_loss + self.balance_loss + self.diversity_loss

        #     if random.random() < 0.1:
        #         logging.info(f"uw_loss {unweighted_loss:.4f}, total_loss {total_loss:.4f}, balance_loss {self.balance_loss:.4f}, diversity_loss {self.diversity_loss:.4f}")
    ...
    def build_lossfn(self):
        ...
        def lossfn(parameters, logits, output):
            ...
            if random.random() < 0.1:
                logging.info(f"main {main_loss:.4f}, balance_loss {self.balance_loss:.4f}, diversity_loss {self.diversity_loss:.4f}")
    ...
    def build_value_only_lossfn(self, device):
        ...
        indexes = list(self.tokenizer.value_indexes())
        def lossfn(parameters, logits, output):
            val_logits = logits[:, :, indexes]
            val_output = torch.zeros_like(output[:, :, indexes]).long()
            val_output[output[:, :, indexes] > 0] = 1
